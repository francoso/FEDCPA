...
def mask_net_weights(global_w, local_critical_mask):
    # 用于存储拉平后的net参数
    flattened_net_weights = []
    # 用于存储拉平后的参数名称（方便后续还原）
    weight_names = []

    # 遍历net的参数字典，拉平参数并记录名称
    for name, param in global_w.items():
        flattened_weight = param.detach().reshape(-1).clone()  # 拉平参数，并通过detach和clone避免梯度等问题
        flattened_net_weights.append(flattened_weight)
        weight_names.append(name)

    # 将拉平后的net参数拼接为一个一维张量（和之前local_vec_critical类似的形式）
    flattened_net_weights_tensor = torch.cat(flattened_net_weights)

    # 进行掩码操作，这里分别生成掩码为1（保留元素）和掩码为0（舍弃元素）的情况
    mask_1 = torch.abs(local_critical_mask) > 0  # 掩码1：只要对应元素不为0就保留
    mask_0 = torch.abs(local_critical_mask) <= 0  # 掩码0：对应元素为0时舍弃

    # 对拉平的net权重应用掩码1
    masked_flattened_net_weights_1 = flattened_net_weights_tensor * mask_1

    # 对拉平的net权重应用掩码0
    masked_flattened_net_weights_0 = flattened_net_weights_tensor * mask_0

    # 拆分掩码1后的权重，准备还原到net中
    split_masked_weights_1 = torch.split(masked_flattened_net_weights_1, [weight.numel() for weight in flattened_net_weights])

    # 拆分掩码0后的权重，准备还原到net中
    split_masked_weights_0 = torch.split(masked_flattened_net_weights_0, [weight.numel() for weight in flattened_net_weights])

    # 还原net参数（掩码1情况）
    restored_net_para_1 = {}
    for name, split_weight_1 in zip(weight_names, split_masked_weights_1):
        original_shape = global_w[name].shape
        restored_weight_1 = split_weight_1.view(original_shape)
        restored_net_para_1[name] = restored_weight_1

    # 还原net参数（掩码0情况）
    restored_net_para_0 = {}
    for name, split_weight_0 in zip(weight_names, split_masked_weights_0):
        original_shape = global_w[name].shape
        restored_weight_0 = split_weight_0.view(original_shape)
        restored_net_para_0[name] = restored_weight_0

    return restored_net_para_1, restored_net_para_0

def global_aggregation(nets_this_round, args, fed_avg_freqs, global_w, party_list_this_round, prev_global_w, prev_prev_global_w, global_model, cur_round, logger, global_w_list, prev_global_w_list, prev_prev_global_w_list):
    ...
    if args.global_defense == 'average':
        ...
        print("global_w_stacked shape",global_w_stacked.shape)
        print("global_topk_indices shape",global_topk_indices.shape)
        ...
        print("w_stacked shape",w_stacked.shape)
        print("local_topk_indices shape",local_topk_indices.shape)
    ...
    # def global_aggregation(nets_this_round, args, fed_avg_freqs, global_w, party_list_this_round, prev_global_w, prev_prev_global_w, global_model, cur_round, logger, global_models, global_w_list):

    elif args.global_defense == 'test':
        # for net_id, net in enumerate(nets_this_round.values()):
        #     net_para = net.state_dict()
        #     if net_id == 0:
        #         for key in net_para:
        ...
        local_w_list = []
        for net_id,net in enumerate(nets_this_round.values()):
            local_w_list.append(copy.deepcopy(net.state_dict()))
        ...
        #计算各个本地模型的关键参数
        tau = 0.50  # 超参数 前50%的参数为关键参数
        for net_id, net in enumerate(nets_this_round.values()):
            net_para = net.state_dict()
            critical_dict = {}
            for name, val in net_para.items():
            #    if val.dim() in [2, 4]: #有待商榷为啥
                critical_weight = torch.abs((val - prev_global_w_list[party_list_this_round[net_id]][name]) * val) #TODO:换成prev_global_w_list
                critical_dict[name] = critical_weight
            local_critical_list.append(get_weight(critical_dict)) #得到所有本地模型的关键参数（list）

        w_stacked = torch.stack(local_critical_list, dim=0)
        local_topk_indices = torch.abs(w_stacked).topk(int(w_stacked.shape[1] * tau)).indices
        #local_bottomk_indices = torch.abs(w_stacked).topk(int(w_stacked.shape[1] * 0.01), largest=False).indices

        local_critical_mask_list = []  #每个模型的本地关键参数掩码
        for net_id, net in enumerate(nets_this_round.values()):
            local_critical_mask = torch.zeros(w_stacked.shape[1])
            print(local_critical_mask[local_topk_indices[net_id]])
            local_critical_mask[local_topk_indices[net_id]] = 1.0
            local_critical_mask_list.append(local_critical_mask)

        #计算关键参数重合率这个计算方式有待商榷，可能要换成相似度
        pairwise_score_total = 0.0
        pairwise_score_max = 0.0
        pairwise_score = np.zeros((len(nets_this_round), len(nets_this_round)))

        for i in range(len(nets_this_round)):
            for j in range(len(nets_this_round)):
                if i == j:
                    pairwise_score[i][j] = 0
                else:
                    pairwise_score[i][j] = 1 - torch.sum(torch.abs(local_critical_mask_list[i]-local_critical_mask_list[j])) / (2 * w_stacked.shape[1])
                    pairwise_score_total += pairwise_score[i][j]
                    if pairwise_score_max < pairwise_score[i][j]:
                        pairwise_score_max = pairwise_score[i][j]

        print(pairwise_score)
        beta = 100  #超参数决定聚类力度
        pairwise_score_avg = pairwise_score_total / ((len(nets_this_round)*(len(nets_this_round)-1)))
        pairwise_thresh = pairwise_score_avg + cur_round/beta * (pairwise_score_max-pairwise_score_avg)
        print(pairwise_thresh)
        ...
        if cur_round >= 10:
            for i in range(len(nets_this_round)):
                ...
                for j in range(len(nets_this_round)):
                    if i==j:
                        cur_cluster_list.append(j)
                    else:
                        if pairwise_score[i][j] >= pairwise_thresh:
                            cur_cluster_list.append(j)
                cluster_list.append(cur_cluster_list)
        else:
            for i in range(len(nets_this_round)):
                cur_cluster_list = []
                for j in range(len(nets_this_round)):
        ...
        for id,cluster in enumerate(cluster_list):
            print("cluster{}:".format(party_list_this_round[id]))
            for i in cluster:
                print("{}".format(party_list_this_round[i]),end=",")
            print(" ")
        #计算每个聚类内的安全得分
        ...
        for net_id, net in enumerate(nets_this_round.values()): #对每一个client聚合一个个性化模型
            local_critical_mask = local_critical_mask_list[net_id]      #这个client的关键参数掩码
            local_w = local_w_list[net_id] #读取这个client的这一轮模型
            critical_weight, noncritical_weight = mask_net_weights(local_w, local_critical_mask)
            critical_weight_update = {}
            noncritical_weight_update = {}
            for key, value in local_w.items():
                #critical_weight_update[key] = torch.tensor(torch.zeros_like(local_w[key]),dtype=torch.float32)
                #noncritical_weight_update[key] = torch.tensor(torch.zeros_like(local_w[key]),dtype=torch.float32)
                critical_weight_update[key] = local_w[key].clone().detach().fill_(0).to(torch.float32)
                noncritical_weight_update[key] = local_w[key].clone().detach().fill_(0).to(torch.float32)
            for cur_net_id, cur_net in enumerate(nets_this_round.values()): # 遍历这个client的聚合
                cur_local_w = local_w_list[cur_net_id]  # 读取这个client的模型
                #cur_local_critical_mask = local_critical_mask_list[cur_net_id]  # 这个client的关键参数掩码
                cur_critical_weight, cur_noncritical_weight = mask_net_weights(cur_local_w, local_critical_mask)
                if cur_net_id ==0:
                    if cur_net_id in cluster_list[net_id]:  # 遍历到的模型是聚类内的个性聚合
                        for key, value in cur_local_w.items():
                            #global_w[key] = cur_critical_weight[key] / len(cluster_list[net_id])
                            global_w[key] = (cur_critical_weight[key] / len(cluster_list[net_id])) * 0.9
                            global_w[key] += cur_noncritical_weight[key] / len(nets_this_round)
                    else:  # 聚类外的共性聚合
                        for key, value in cur_local_w.items():
                            global_w[key] = (cur_critical_weight[key] / (len(nets_this_round)-len(cluster_list[net_id]))) * 0.1
                            global_w[key] += cur_noncritical_weight[key] / len(nets_this_round)
                #if cur_net_id == 0:
                #    for key in cur_critical_weight:
                #        global_w[key] = (cur_critical_weight[key] + cur_noncritical_weight[key]) / len(nets_this_round)
                #else:
                #    for key in cur_critical_weight:
                #        global_w[key] += (cur_critical_weight[key] + cur_noncritical_weight[key]) / len(nets_this_round)
                else:
            ...
            #    global_w[key] = (critical_weight[key] + alpha * critical_weight_update[key]) + (noncritical_weight[key] + alpha2 * noncritical_weight_update[key])
            global_w_list[party_list_this_round[net_id]] = copy.deepcopy(global_w)
    return global_w, global_w_list
