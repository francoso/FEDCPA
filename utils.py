...
def compute_accuracy(model, dataloader, get_confusion_matrix=False, device="cpu", multiloader=False):
    ...
    # true_labels_dict = {}
    # for i in true_labels_list:
    #     true_labels_dict[i] = true_labels_list.tolist().count(i)
    # for i in sorted(true_labels_dict):
    #     print("{}:{}".format(i,true_labels_dict[i]),end=" ")
    # print(" ")
...
def compute_accuracy_vote(models, dataloader, get_confusion_matrix=False, device="cpu", multiloader=False):
    was_training = False
    ...
    if multiloader:
        ...
        pred_labels_list_all = np.array([])
        pred_labels_list_vote =  np.array([])
        with torch.no_grad():
            for model in models.values():
                true_labels_list, pred_labels_list = np.array([]), np.array([])
                for batch_idx, (x, target) in enumerate(dataloader):
                    ...
                    if device == "cpu":
                        ...
                        true_labels_list = np.append(true_labels_list, target.data.cpu().numpy())
                avg_loss = sum(loss_collector) / len(loss_collector)
                pred_labels_list_all = np.append(pred_labels_list_all,pred_labels_list)

            pred_labels_list_all = pred_labels_list_all.reshape(-1,len(pred_labels_list))
            pred_labels_list_all_trans = pred_labels_list_all.T
            for pred_labels in pred_labels_list_all_trans:
                pred_labels_list_vote = np.append(pred_labels_list_vote, np.argmax(np.bincount(pred_labels.astype(int))))

            print(pred_labels_list_vote)
            correct, total = 0, 0
            for batch_idx, (x, target) in enumerate(dataloader):
                if device != 'cpu':
                    x, target = x.cuda(), target.to(dtype=torch.int64).cuda()
                pred_label = pred_labels_list_vote[batch_idx*32:batch_idx*32+32]
                target_np = target.detach().cpu().numpy()
                # 逐元素比较两个数组，得到布尔型数组（对应元素相等为True，不等为False）
                compare_result = pred_label == target_np
                # 对布尔型数组求和（True视为1，False视为0），得到相等元素的个数
                total += x.data.size()[0]
                correct += np.sum(compare_result)
