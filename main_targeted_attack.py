...
def get_args():
    ...
    parser.add_argument('--comm_round', type=int, default=150, help='number of maximum communication roun')
...
global_models = [global_models[0]] * args.n_parties


n_comm_rounds = args.comm_round
global_w_list = [global_model.state_dict()] * args.n_parties
#global_w_list = []
#for net in nets.values():
#    global_w_list.append(copy.deepcopy(net.state_dict()))
prev_global_w_list = []
prev_prev_global_w_list = []
...
for round in range(n_comm_rounds):
    ...
    if round != 0:
        ...
        prev_prev_global_w_list = copy.deepcopy(prev_global_w_list)
        prev_global_w_list = copy.deepcopy(global_w_list)
    ...
    for net_id,net in enumerate(nets_this_round.values()):
        net.load_state_dict(global_w_list[party_list_this_round[net_id]])
    ...
    if round == 0:
        ...
        for i in range(args.n_parties):
            prev_prev_global_w_list.append(copy.deepcopy(global_model.state_dict()))
            prev_global_w_list.append(copy.deepcopy(global_model.state_dict()))
    ...
    global_w,global_w_list = global_aggregation(nets_this_round, args, fed_avg_freqs, global_w, party_list_this_round, prev_global_w, prev_prev_global_w, global_model, round, logger, global_w_list, prev_global_w_list, prev_prev_global_w_list)
    ...
    for net_id, net in enumerate(nets_this_round.values()):
        ...
        logger.info('>> Global Model %i Test accuracy: %f' % (party_list_this_round[net_id],test_acc))

        asr_acc,  _ = compute_accuracy(net, test_dl_poisoned, device=device)
        logger.info('>> Global Model %i Attack Success Rate: %f' % (party_list_this_round[net_id],asr_acc))

        net.to('cpu')
        global_models[party_list_this_round[net_id]] = net

    test_acc, conf_matrix, _ = compute_accuracy_vote(nets, test_dl, get_confusion_matrix=True, device=device)
    logger.info('>> Global Model Vote Test accuracy: %f' % (test_acc))
    asr_acc, _ = compute_accuracy_vote(nets, test_dl_poisoned, device=device)
